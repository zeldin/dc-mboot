...
static unsigned int find_datatrack(struct TOC *toc, int session)
{
  int i, first, last;
  if(session && (first=TOC_TRACK(toc->first))>=1 && first<=99 &&
     (last=TOC_TRACK(toc->last))>=first /*&& TOC_CTRL(toc->entry[first-1]==4)*/)
    return TOC_LBA(toc->entry[first-1]);
  else {
...
static int find_root(int session, unsigned int *psec, unsigned int *plen)
{
  ...
  if((r=init_drive())!=0)
    return r;
  if((r=read_toc(&toc, session))!=0)
    return r;
  if(!(sec = find_datatrack(&toc, session)))
    return ERR_DIRERR;
...
int load_ip(int session, unsigned char *buf)
{
  static struct TOC toc;
  int r;
  unsigned int sec;
  ...
  if(!(sec = find_datatrack(&toc, session)))
    return ERR_DIRERR;
  if((r=read_sectors((char *)buf, sec, 16))!=0)
    return r;
  return 0;
}
...
int open(int session, const char *path, int oflag)
{
  int fd, r;
  ...
  if((r=find_root(session, &sec, &len)))
    return r;
...
DIR *opendir(int session, const char *dirname)
{
  /* Try to open a handle for the directory */
  if((g_dir.dd_fd = open(session, dirname, O_DIR|O_RDONLY)))
    return NULL;
